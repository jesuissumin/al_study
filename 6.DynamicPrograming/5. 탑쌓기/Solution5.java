...
class Solution5 {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int test_case = 1; test_case <= 10; test_case++) {
			...
			// 		if (prevH >= 0) {
			// 			for (int k=0; k<j-1; k++){
			// 				c[i][j] = (c[i][j]+c[prevH][k])%1000000;
			// 			}
			// 		}
			// 		if ((prevH+d[j-1])>=0) {
			// 			c[i][j] = (c[i][j]+c[prevH+d[j-1]][j-1])%1000000;
			// 		}
			// 	}
			// }
			...
			// Answer = 0;
			// for (int i=0; i<n; i++){
			// 	Answer = (Answer+c[H][i])%1000000;
			// }
		
			//===============================
			// 시도 2 : H가 아닌 n에 대한 최적부분구조로 문제 나누기 
			//===============================
